...
try{
  for(const viewport of [{width:1365,height:1000},{width:390,height:844}]){
    ...
    await page.getByRole('button',{name:'Mijn reserveringen',exact:true}).click();
    ...
    assert.deepEqual(errors,[]);passed++;console.log('PASS desktop/mobiel '+viewport.width+': gast, login-testdubbel, extern verzoek, admin goedkeuring, support/reactie, CSV import/dubbel import, Excel, intern boeken/wijzigen, XSS-regressie, uitloggen en layout');
    await page.close();
  }
